...
include_in_table <- reactive({
  map_data() %>%
    select(providername, address, city, state, zip, information, offers)
})
...
output$map <- renderLeaflet({
  ...
  leaflet(width = "80%") %>%
    ...
    addMarkers(
      ...
      lng = ~lon, lat = ~lat,
      popup = paste(
        "<b><a href='", map_data()$web_link, "'>", map_data()$providername, "</a></b><br/>",
        "<b>Address:</b>", map_data()$address, ",", map_data()$city, ",", map_data()$state, ",",
        map_data()$zip, "<br/>", "<b>Phone:</b> ", map_data()$contact, "<br/><b>Notes:</b> ", map_data()$information, "<br/>", map_data()$offers
      ),
...
output$table <- renderReactable({
  reactable(include_in_table(),rownames = FALSE, resizable = TRUE,
            ...
            columns = list(
              providername = colDef(name = "Provider"),
              address = colDef(name = "Address"),
              city = colDef(name = "City"),
              state = colDef(name = "State"),
              zip = colDef(name = "Zip"),
              information = colDef(name = "Notes"),
              offers = colDef(name = "Addtl. Notes")
            ))
})
